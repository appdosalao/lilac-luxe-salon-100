-- Correção de Segurança: Adicionar validação de acesso à função converter_agendamento_online
-- Issue: converter_agendamento_definer - Privilege Escalation
-- A função permite que qualquer usuário autenticado converta agendamentos para qualquer user_id
...
BEGIN
  -- VALIDAÇÃO DE SEGURANÇA: Verificar se o usuário autenticado é o dono do user_id
  IF user_id != auth.uid() THEN
    RAISE EXCEPTION 'Não autorizado: você só pode converter agendamentos para sua própria conta';
  END IF;
