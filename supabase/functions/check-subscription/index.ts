...
serve(async (req) => {
  ...
  try {
    ...
    if (userError || !user) {
      ...
      // Retornar resposta 200 com status inactive em vez de erro 500
      return new Response(JSON.stringify({
        subscribed: false,
        status: 'inactive',
        product_id: null,
    ...
    let trialEnd = null;
    let status: 'trial' | 'active' | 'expired' | 'inactive' = 'inactive';
    ...
    if (hasActiveSub) {
      ...
      logStep("Determined subscription tier", { productId, status });
    } else {
    ...
    logStep("ERROR: Unexpected exception in check-subscription", { message: errorMessage });
    // Even on unexpected errors, return inactive status instead of 500
    return new Response(JSON.stringify({
      subscribed: false,
      status: 'inactive',
      product_id: null,
